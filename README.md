...
rgArenaFlush(&a);                  /* durable on disk      */
rgArenaDestroy(&a);
...
- Hash function is derived from **wyhash** (Wang Yi, public domain) — a 64×64→128 multiply mixer. The mix step is *protected* (as in rapidhash's protected mode): the multiplicands are XORed back into the result, so an input that zeroes one multiplicand cannot erase the rest of the key or the seed. Plain wyhash maps a ~2⁶⁴-key class to digest 0 under every seed, which would defeat both `HashIndex`'s two-seed identity and the descent reseed. Output is therefore not bit-compatible with upstream wyhash. It is both faster and better-distributed than FNV-1a, especially in the top bits the trie consumes 2 at a time. Unaligned reads use `__unaligned` on MSVC (compiler keyword, not a libc symbol) and the byte-OR pattern on Clang/GCC (recognised as a single mov on x86/x64).
